...
- 구성: 히어로 이미지, CTA 버튼.
- 히어로 이미지는 Next.js `Image` 컴포넌트에 `priority` 설정으로 최우선 로드 (CLS 방지).
  - `width={941}`, `height={660}` 고정 비율.
  - `w-[120%]` + `max-w-[941px]` + `xl:w-[50%]` 반응형 크기.
...
- Hero 섹션 높이 정책
  ...
  - Hero 내부 콘텐츠(이미지/CTA)는 섹션 내 세로 중앙 정렬.
- 반응형 기준
  - Mobile: `pt-140 / pb-80`
  - Desktop: `pt-230 / pb-130`
